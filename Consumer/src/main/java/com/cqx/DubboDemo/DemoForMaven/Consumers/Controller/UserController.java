...
public class UserController {
    ...
    public String postEdit(Model model,  User postUser, BindingResult bindingResult
            ,@RequestParam(required = false) String searchQueryStr) {

        System.out.println("enter postEdit");

        if (!Utils.isValidMobile(postUser.getMobile())){
            bindingResult.rejectValue("mobile", "code", "不合法的手机号码，请重新输入!");
        }
        if (!Utils.isValidEmail((postUser.getEmail()))){
            bindingResult.rejectValue("email", "code", "不合法的邮箱,请重新输入!");
        }
        ...
        if (bindingResult.hasErrors()) {
            ...
            System.out.println("searchQueryStr = "+searchQueryStr);
            return "user/edit";
        }
    ...
    public String postCreateUser(Model model, User postUser, BindingResult bindingResult) {

        if (!Utils.isValidMobile(postUser.getMobile())){
            bindingResult.rejectValue("mobile", "code", "不合法的手机号码，请重新输入!");
        }
        else if (userService.isExistMobile(postUser)) {
        ...
        else if (userService.isExistEmail(postUser)) {
